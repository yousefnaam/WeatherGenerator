...
int l = 0;
        ...
        for(int i = 0; i < drywet.length; i++)
        {
            if(drywet[i][0] == latitude && drywet[i][l+1] == longitude)
            {
                for(int x=0; x < drywetProbability.length; x++)
                {
                    drywetProbability[x] = drywet[i][l+2];
                }
                l++;
            }
        }
        ...
        {
            if(wetwet[i][l] == latitude && wetwet[i][l+1] == longitude)
            {
                for(int x=0; x < wetwetProbability.length; x++)
                {
                    wetwetProbability[x] = wetwet[i][l+2];
                }
                l++;
            }
        }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        int temp;
        int first = StdRandom.uniform(0,1);
        if(first >= 0.5)
        arr[0] = 1;
        else
        arr[0]=2;
...
return count;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
       ...
       {
        if(forecast[i]== mode && forecast[i]==forecast[i+1])
        {
        count++;
        }
        else
